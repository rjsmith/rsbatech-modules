...
struct OscelotTextField : LedDisplayTextField {
	...
	OscelotTextField() {
		...
		bgColor = color::BLACK;
		bgColor.a = 0.3;
		fontPath = asset::plugin(pluginInstance, "res/fonts/RobotoMono-Medium.ttf");
	}
	...
	void drawLayer(const DrawArgs& args, int layer) override {
		if (layer == 1) {
			...
			if (font->handle >= 0) {
				...
				bndIconLabelCaret(args.vg, textOffset.x, textOffset.y, box.size.x - 2 * textOffset.x, box.size.y - 2 * textOffset.y, -1, color, textSize, text.c_str(),
	...
	void onDoubleClick(const event::DoubleClick& e) override { doubleClick = true; }
